...
class FileSystemModel(QFileSystemModel):
    ...
    def highlight(self, index, condition, color):
        ...
        if index.isValid():
            if not any(
                d.get("condition") == self.condition for d in self.highlighted_indexes
            ):
                self.highlighted_indexes.append(
                    {"index": index, "condition": condition}
                )
                if len(self.highlighted_indexes) > 3:
                    # Or: self.highlighted_indexes = self.highlighted_indexes[-3:]
                    self.highlighted_indexes.pop(
                        0
                    )  # Store only the last X of them, remove the others
            # Data changed signal
