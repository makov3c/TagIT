    ...
    * Markdown in HTML
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        self.estate.value_changed.connect(
            ...
            lambda v: self.editor_state_update_handler(v)
        )  # v
        ...
        source = (
            Source(self.settings.source) if self.settings.source > 0 else Source.HTML
        )
        ...
        # Async highlighter
        self.async_highlighter = AsyncHighlighter(
            callback=lambda is_full: self.rehighlight_editor(is_full)
        )
        ...
        if (
            not os.path.exists(app_config_path)
            and not os.access(os.path.dirname(app_config_path), os.W_OK)
            or not os.access(AppConfig().get_app_config_path(), os.W_OK)
        ):
            MessageBox(
                text=self.lexemes.get(
                    "message_app_config_file_access",
                    file_path=AppConfig().get_app_config_path(),
                ),
                icon_type=2,
                parent=self,
            )
    ...
    def init_font(self):
        ...
        self.logger.debug(
            "Font point size set to %d, font size ratio %.2f"
    ...
    def settings_update_handler(self, data: dict) -> None:
        ...
        if (
            any(
                item
                in ["viewer_highlight_todos", "viewer_process_emojis"]  # Check item
        ...
        if self.get_mode() == Mode.EDIT:
            ...
            if hasattr(self, "text_view"):
                if data["c"] == Mode:
                    ...
                    self.store_doc_cursor_pos(
                        mode=data["pv"]
                    )  # Or just: self.get_prev_mode()
                # Hide widget
                self.text_view.hide()
            ...
            if hasattr(self, "text_view"):
                ...
                if data["c"] == Mode:
                    ...
                    self.restore_doc_cursor_pos(
                        mode=data["pv"], source_widget=self.text_view
                    )
            # Clear search fields and statuses
            self.action_search_clear()
    ...
    def init_encrypt_helper(
        ...
        encrypt_helper = EncHelper(
            enc_password=self.enc_password, salt=salt, iterations=iterations
        )
    ...
    def get_encrypt_helper(
        ...
        if encrypt_helper is None:
            ...
            encrypt_helper = self.init_encrypt_helper(
                self.enc_password, salt, iterations
            )
    ...
    def init_ui(self) -> None:
        ...
        """
        if (
            self.settings.ui_pos_x > 0
            and self.settings.ui_pos_y > 0
        ...
        if not position:
            ...
            splitter_width_col = int(
                (
                    ...
                    - splitter.handleWidth() * (splitter.count() - 1)
                )
                / self.AREA_WEIGHT
            )  # Fallback to default proportions
            splitter.setSizes(
                [
                    self.AREA_WEIGHT_TREE * splitter_width_col,
                    self.AREA_WEIGHT_EDIT * splitter_width_col,
                ]
            )
        else:
    ...
    def init_palette(self):
        ...
        conf_map = {
            QPalette.ColorRole.Window: self.theme_helper.get_color(
                "color_palette_window"
            ),
            QPalette.ColorRole.WindowText: self.theme_helper.get_color(
                "color_palette_window_text"
            ),
            QPalette.ColorRole.Base: self.theme_helper.get_color(
                "color_palette_base"
            ),  # For background
            ...
            QPalette.ColorRole.AlternateBase: self.theme_helper.get_color(
                "color_palette_alternate_base"
            ),
            QPalette.ColorRole.ToolTipBase: self.theme_helper.get_color(
                "color_palette_tooltip_base"
            ),
            QPalette.ColorRole.ToolTipText: self.theme_helper.get_color(
                "color_palette_tooltip_text"
            ),
            QPalette.ColorRole.Text: self.theme_helper.get_color("color_palette_text"),
            QPalette.ColorRole.Button: self.theme_helper.get_color(
                "color_palette_button"
            ),
            QPalette.ColorRole.ButtonText: self.theme_helper.get_color(
                "color_palette_button_text"
            ),
            # The top and left sides (simulating light falling from the top/left)
            QPalette.ColorRole.Light: self.theme_helper.get_color(
                "color_palette_light"
            ),
            ...
            QPalette.ColorRole.Midlight: self.theme_helper.get_color(
                "color_palette_mid_light"
            ),
            QPalette.ColorRole.Shadow: self.theme_helper.get_color(
                "color_palette_shadow"
            ),
            # Stand out against other text for emphasis and is typically used against darker backgrounds
            QPalette.ColorRole.BrightText: self.theme_helper.get_color(
                "color_palette_bright_text"
            ),
            QPalette.ColorRole.Link: self.theme_helper.get_color(
                "color_palette_link"
            ),  # For hyperlinks
            QPalette.ColorRole.LinkVisited: self.theme_helper.get_color(
                "color_palette_link_visited"
            ),  # For hyperlinks
            # This role defines the color used to indicate selected items or current focus in widgets
            QPalette.ColorRole.Highlight: self.theme_helper.get_color(
            ...
            QPalette.ColorRole.HighlightedText: self.theme_helper.get_color(
                "color_palette_highlighted_text"
            ),
            # For input field placeholder
            QPalette.ColorRole.PlaceholderText: self.theme_helper.get_color(
                "color_palette_placeholder_text"
            ),
            QPalette.ColorRole.Accent: self.theme_helper.get_color(
    ...
    def set_app_title(self, sub_title: str = None):
        ...
        # Format sub-title
        sub_title_f = (
            sub_title[: max_sub_length - 3] + "..."
            if sub_title and len(sub_title) > max_sub_length
            else sub_title
        )
        app_title = self.lexemes.get("app_title")
        # Title with a sub_title if exists
        if sub_title_f and len(sub_title_f) > 0:
            title = self.lexemes.get(
                "app_title_with_sub", app_title=app_title, sub_title=sub_title_f
            )
        else:
            # App title
            title = app_title
    ...
    def get_editor_container(self) -> QWidget:
        if hasattr(self, "editor_container") and isinstance(
    ...
    def create_navigation_panel(self) -> Union[FileTree, QWidget]:
        ...
        self.file_model.setRootPath(
            QDir.currentPath()
        )  # Base directory for file system model
        self.file_model.setFilter(
            QDir.Filter.NoDot | QDir.Filter.Dirs | QDir.Filter.Files
        )  # Hide '.' but keep '..'
        self.file_model.setNameFilterDisables(
            False
        )  # Ensure filtered results are shown properly
        ...
        watch_path = (
            QDir.currentPath()
        )  # Consider using self.get_tree_active_dir() for dynamic tracking
        self.file_watcher.addPath(
            watch_path
        )  # Watch for changes in the selected directory
    ...
    def get_tree_active_dir(self):
        ...
        if not active_path:
            ...
            active_path = (
                self.tree_active_dir if self.tree_active_dir else QDir.currentPath()
            )
        return active_path
    ...
    def show_tree_context_menu(self, tree_view: QListView, pos: QPoint) -> None:
        ...
        if file_path and not os.path.exists(file_path):
            self.logger.warning(
                'Trying to create context menu on an element that does not exist "%s"'
                % file_path
            )
            return
    ...
    def rename_file_dialog(self, file_path: str) -> None:
        ...
        * QDialog.DialogCode.Rejected
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            self.rename_file_dialog_callback(
    ...
    def rename_file_dialog_callback(
        self,
        ...
        if not os.path.isfile(to_file_path):
            if file_helper.is_writable_path(
                from_file_path
            ) and file_helper.is_writable_path(to_file_path):
                os.rename(from_file_path, to_file_path)
                if file_helper.is_file_openable(to_file_path):
                    self.load_file(to_file_path)
                else:
                    self.logger.warning(
                        f"Permission denied when accessing the file {to_file_path}"
                    )
                    MessageBox(
                        text=self.lexemes.get("open_file_permission_error"),
                        icon_type=2,
                        parent=self,
                    )
            else:
                self.logger.warning(
                    f"Permission denied when renaming the file {from_file_path} to {to_file_path}"
                )
                MessageBox(
                    text=self.lexemes.get("rename_file_permission_error"),
                    icon_type=2,
                    parent=self,
                )
        else:
            self.logger.debug(
                'File with the same name is already exists "%s"' % to_file_path
            )
            MessageBox(
                text=self.lexemes.get("dialog_file_rename_warning_exists"),
                icon_type=2,
                parent=self,
            )
    ...
    def delete_file_dialog(self, file_path: str) -> None:
        ...
        self.common_dialog(
            ...
            callback=lambda dialog_callback: self.delete_file_dialog_callback(
                dialog_callback, file_path
            ),
        )

    def delete_file_dialog_callback(
        self, callback: Callable[..., Any], file_path: str
    ) -> None:
        """
        ...
        self.logger.debug(
            'Delete file "%s" dialog callback with sub-callback %s'
            % (file_path, callback)
        )
        ...
        if os.path.isfile(file_path):
            ...
            i = 0
            while (
                del_file_path := file_helper.res_path(
                    del_filename_tpl % (file_path, i if i > 0 else "")
                )
            ...
            if not os.path.isfile(del_file_path):
                if file_helper.is_writable_path(
                    file_path
                ) and file_helper.is_writable_path(del_file_path):
                    os.rename(file_path, del_file_path)
                    self.logger.debug(
                        'File (reversibly) deleted to "%s"', del_file_path
                    )
                else:
                    self.logger.warning(
                        f"Permission denied when renaming the file {file_path} to {del_file_path}"
                    )
                    MessageBox(
                        text=self.lexemes.get("rename_file_permission_error"),
                        icon_type=2,
                ...
                self.logger.debug(
                    'Cannot delete file, error occurred "%s"' % del_file_path
                )
                MessageBox(
                    text=self.lexemes.get("dialog_file_delete_error"),
                    icon_type=2,
                    parent=self,
                )
            ...
            MessageBox(
                text=self.lexemes.get("dialog_file_delete_error_not_found"),
                icon_type=2,
                parent=self,
            )
        if callable(callback):
    ...
    def delete_completely_file_dialog(self, file_path: str) -> None:
        ...
        self.common_dialog(
            ...
            self.lexemes.get(
                name="dialog_file_delete_completely_text", file_name=file_name
            ),
            callback=lambda dialog_callback: self.delete_completely_file_dialog_callback(
                dialog_callback, file_path
            ),
        )

    def delete_completely_file_dialog_callback(
        self, callback: Callable[..., Any], file_path: str
    ) -> None:
        """
        ...
        self.logger.debug(
            'Delete file completely "%s" dialog callback with sub-callback %s'
            % (file_path, callback)
        )
        ...
        if os.path.isfile(file_path):
            try:
                ...
                self.logger.warning(
                    f"Error: {e.strerror}. Could not delete {file_path}."
                )
                MessageBox(
                    text=self.lexemes.get("dialog_file_delete_error"),
                    icon_type=2,
    ...
    def restore_file_dialog(self, file_path: str) -> None:
        ...
        if file_extension:
            ...
            self.logger.warning(
                f"Trying to restore file that has no proper extension set {file_path}"
            )
            return
        ...
        self.common_dialog(
            ...
            callback=lambda dialog_callback: self.restore_file_dialog_callback(
                dialog_callback, from_file_path=file_path, to_file_path=new_file_path
            ),
        )

    def restore_file_dialog_callback(
        self,
        ...
        self.logger.debug(
            "Restore file dialog callback with sub-callback %s" % callback
        )
        ...
        if not os.path.isfile(to_file_path):
            ...
            MessageBox(
                text=self.lexemes.get(
                    "dialog_file_restore_warning_exists", file_name=to_file_path
                ),
                icon_type=2,
                parent=self,
            )
    ...
    def get_any_file(self) -> Union[str, None]:
        ...
        root_index = self.tree_view.rootIndex()
        self.logger.debug(
            'Root files index "%s"', self.tree_proxy_model.data(root_index)
        )
    ...
    def enc_password_dialog(self, hint: str = None) -> Union[EncPassword, None]:
        ...
        dialog = EncPasswordDialog(
            hint=hint, parent=self
        )  # or just `QInputDialog(self)`
    ...
    def get_enc_password_dialog_cnt(self) -> Union[int, None]:
        return (
            self.enc_password_dialog_cnt
            if hasattr(self, "enc_password_dialog_cnt")
            else None
        )
    ...
    def common_dialog(
        ...
        text: str,
        callback: Callable[..., Any],
        reject_callback: Callable[..., Any] = None,
    ) -> None:
        """
    ...
    def create_editor_panel(self) -> EditWidget:
        ...
        edit_widget.content_set.connect(
            lambda:
        ...
        self.logger.debug(
            "Edit widget geometry %d x %d"
            % (
                edit_widget.frameGeometry().width(),
                edit_widget.frameGeometry().height(),
            )
        )
        ...
        self.logger.debug(
            f'Edit widget created, signals was un-blocked "{was_blocked}"'
        )
    ...
    def open_link_dialog_proxy(self):
        ...
        if self.settings.viewer_open_link_confirmation:
            ...
            return lambda url: (
                ...
                self.check_local_link(url, True)
                if self.check_local_link(url)
                else self.common_dialog(
    ...
    def check_local_link(self, url: QUrl, execute: bool = False) -> Any:
        ...
        if url.toString().startswith("expandable") or url.toString().startswith(
            "collapsible"
        ):
            return True
        ...
        try:
            ...
            self.logger.warning(
                f"Permission denied when accessing the file: {url.toString()}"
            )
            is_file = False
        except OSError as e:
    ...
    def create_view_panel(self) -> QTextBrowser:
        ...
        # This event is emitted when edit widget document has been set
        view_widget.content_set.connect(
            lambda:
            # Restore cursor and asure text widget has a focus
            (
        ...
        self.logger.debug(
            "Web view geometry %d x %d"
            % (
                view_widget.frameGeometry().width(),
                view_widget.frameGeometry().height(),
            )
        )
        ...
        self.logger.debug(
            f'View widget created, signals was un-blocked "{was_blocked}"'
        )
        ...
        async def cleanup_tasks():
            ...
            for i, task in enumerate(tasks):
                ...
                if (
                    ...
                    and not (
                        hasattr(task, "get_coro")
                        and task.get_coro().__qualname__ == "Event.wait"
                    )
                ):
                    ...
                    try:
                        ...
                        self.logger.info(
                            f"[{i + 1}/{tasks_total}] Pending task {task.get_name()} "
    ...
    def rehighlight_editor(self, full_rehighlight: bool = False) -> None:
        ...
        was_blocked = edit_widget.blockSignals(True)
        self.logger.debug(
            f'Re-highlighting the text > signals was blocked "{was_blocked}"'
        )
    ...
    def on_modification_changed(self, changed) -> None:
        ...
        self.logger.debug(
            "Modification changed (%s) signal from %s" % (changed, self.sender())
        )
    ...
    def on_block_count_changed(self, new_block_count: int) -> None:
        ...
        self.logger.debug(
            "Block count (%d) changed signal from %s" % (new_block_count, self.sender())
        )
    ...
    def resizeEvent(self, event) -> None:
        ...
        * https://doc.qt.io/qt-6/qwidget.html#resizeEvent
        ...
        self.logger.debug(
            "%s > App window geometry %d x %d"
            % (self.__class__.__qualname__, ui_width, ui_height)
        )
        self.logger.debug(
            "%s > Web view geometry  %d x %d"
    ...
    def moveEvent(self, event) -> None:
        ...
        * https://doc.qt.io/qt-6/qwidget.html#moveEvent
        ...
        self.logger.debug(
            "%s > App window position %s" % (self.__class__.__qualname__, self.pos())
        )
    ...
    def get_cursor_label_text(self, selected_text: str = None) -> str:
        ...
        if selected_text:
            if self.settings.show_global_cursor_position:
                ...
                return self.lexemes.get(
                    "statusbar_cursor_label_selected_with_global",
                    scope="statusbar",
                    line=self.line_num_hr,
                    column=self.col_num,
                ...
                return self.lexemes.get(
                    "statusbar_cursor_label_selected",
                    scope="statusbar",
                    line=self.line_num_hr,
                    column=self.col_num,
                    selected=len(selected_text),
            ...
            if self.settings.show_global_cursor_position:
                ...
                return self.lexemes.get(
                    "statusbar_cursor_label_with_global",
                    scope="statusbar",
                    line=self.line_num_hr,
                    column=self.col_num,
                    position=self.cursor_pos,
    ...
    def action_new_file(self, content: str = None) -> bool:
        ...
        if not content:
            new_file_content_tpl = (
                "# "
                + self.lexemes.get("action_new_file_first_line_template_text")
                + " %d\r\n"
            )
    ...
    def action_source(self) -> None:
        ...
        self.logger.debug(
            "Source view requested from mode '%s', from source '%s';"
            "previous states were '%s' and '%s' correspondingly"
            % (
                Mode(self.get_mode()).name,
                Source(self.get_source()).name,
                Mode(self.get_prev_mode()).name,
                Source(self.get_prev_source()).name,
            )
        )
    ...
    def action_encrypt(self) -> None:
        ...
        if self.is_file_encrypted(current_file_path):
            MessageBox(
                text=self.lexemes.get("encrypt_file_warning_file_is_already_encrypted"),
                icon_type=2,
                parent=self,
            )
            return
        ...
        self.common_dialog(
            ...
            self.lexemes.get(
                name="dialog_encrypt_file_text", file_name=current_file_name
            ),
            callback=lambda dialog_callback: self.encrypt_file_dialog_callback(
                dialog_callback, current_file_path, encrypted_file_path
            ),
        )

    def encrypt_file_dialog_callback(
        self,
        callback: Callable[..., Any] = None,
        from_file_path: str = None,
        to_file_path: str = None,
        ignore_existing_to_file: bool = False,
    ) -> None:
        """
        ...
        self.logger.debug(
            "Encrypt file dialog callback with sub-callback %s" % callback
        )
        ...
        if os.path.isfile(to_file_path) and not ignore_existing_to_file:
            self.common_dialog(
                ...
                self.lexemes.get(
                    name="dialog_encrypt_file_rewrite_existing_text",
        ...
        if (
            file_header
            is None  # File_header should exist at this point, but check if it is not None anyway
        ...
        encrypted_file_body_b = self.get_encrypt_helper(
            salt=file_salt, new_password=True, iterations=file_iterations
        ).encrypt_data(file_body.encode("utf-8"))
        if encrypted_file_body_b:
            encrypted_file_body = encrypted_file_body_b.decode("utf-8")
            # Check and adjust header params if needed
            if (
                not file_header.get_enc_param("hint")
    ...
    def action_decrypt(self) -> None:
        ...
        if not self.is_file_encrypted(current_file_path):
            MessageBox(
                text=self.lexemes.get("decrypt_file_warning_file_is_not_encrypted"),
                icon_type=2,
                parent=self,
            )
            return
        ...
        self.common_dialog(
            ...
            self.lexemes.get(
                name="dialog_decrypt_file_text", file_name=current_file_name
            ),
            callback=lambda dialog_callback: self.decrypt_file_dialog_callback(
                dialog_callback, current_file_path, decrypted_file_path
            ),
        )

    def decrypt_file_dialog_callback(
        self,
        callback: Callable[..., Any] = None,
        from_file_path: str = None,
        to_file_path: str = None,
        ...
        self.logger.debug(
            "Decrypt encrypted file dialog callback with sub-callback %s" % callback
        )
        ...
        if os.path.isfile(to_file_path) and not ignore_existing_to_file:
            self.common_dialog(
                ...
                self.lexemes.get(
                    name="dialog_decrypt_file_rewrite_existing_text",
                    file_path=to_file_path,
                ),
                callback=lambda dialog_callback: self.decrypt_file_dialog_callback(
                    # Because of nested dialogs, and each has its own callback
        ...
        try:
            decrypted_file_body = self.get_encrypt_helper(
                salt=file_salt, iterations=file_iterations
            ).decrypt_data(encrypted_file_body.encode("utf-8"))
    ...
    def reset_settings_dialog_callback(
        ...
        self.logger.debug(
            "Reset settings dialog callback with sub-callback %s" % callback
        )
    ...
    def load_file(self, file_path: str) -> bool:  # noqa: C901 - consider simplifying this method
        ...
        if current_file_path is not None and file_path != current_file_path:
            ...
            in_transit_color = self.theme_helper.get_color(
                "main_tree_file_highlighted_in_transit", True
            )
            self.file_model.highlight(
                self.file_model.index(current_file_path),
        ...
        if self.is_file_encrypted(file_path):
            ...
            try:
                decrypted_data = self.get_encrypt_helper(
                    salt=file_salt,
                    hint=file_header.get_enc_param("hint"),
                    iterations=file_iterations,
                ).decrypt_data(file_body.encode("utf-8"))
                ...
                if self.get_encryption() == Encryption.ENCRYPTED:
                    self.logger.debug(
                        "Wrong encryption password but another encrypted file is opened"
                    )
                ...
                # Show error message and reset password only if the password set.
                    ...
                    MessageBox(
                        text=self.lexemes.get(
                            "load_file_encryption_password_incorrect"
                        ),
                        icon_type=3,
                        parent=self,
                    )
                    ...
                    self.logger.debug(
                        "Do nothing with the wrong password but check the loaded page"
                    )
                    """
                ...
                if current_file_path is not None:
                    ...
                    default_color = self.theme_helper.get_color(
                        "main_tree_background", True
                    )
                    self.file_model.highlight(
                        self.file_model.index(current_file_path),
                        os.path.basename(current_file_path),
        ...
        # Load fields content
        if (
            file_header or file_body
        ):  # File body equal to None when file is just created but the header was removed
            self.load_content(file_header, file_body if file_body else "")
        else:
            MessageBox(
                text=self.lexemes.get("load_file_none_content_error"),
                icon_type=2,
                parent=self,
            )
            return False
        ...
        if current_file_path is not None:
            ...
            prev_opened_color = self.theme_helper.get_color(
                "main_tree_file_highlighted_prev_opened", True
            )
            self.file_model.highlight(
                self.file_model.index(current_file_path),
                os.path.basename(current_file_path),
    ...
    def save_active_file(
        self,
        clear_after: bool = False,  # noqa: C901
        allow_save_empty_content: bool = None,
    ) -> Union[bool, None]:
        """
        ...
        self.logger.debug(
            f"Save active file '{current_file_path}' (clear field after: '{clear_after}')"
        )

        # Handle the case where the file no longer exists and cannot be saved
        if not os.path.exists(current_file_path) and not os.access(
            os.path.dirname(current_file_path), os.W_OK
        ):
            self.logger.warning(
                f"Cannot save active file '{current_file_path}', check if it was moved or deleted"
            )
            self.toggle_save_timer(state=False)
            MessageBox(
                text=self.lexemes.get("save_active_file_error_occurred"),
                icon_type=2,
            ...
            self.common_dialog(
                ...
                callback=lambda dialog_callback: (
                    self.save_active_file(
                        clear_after=clear_after, allow_save_empty_content=True
                    ),
        ...
        # Save if any changes
        if (
            file_content or self.estate.allow_save_empty
        ) and self.content != file_content:
            ...
            if self.get_encryption() == Encryption.ENCRYPTED:
                ...
                encrypted_content_b = self.get_encrypt_helper(
                    salt=file_salt, iterations=file_iterations
                ).encrypt_data(content.encode("utf-8"))
    ...
    def load_content_html(self, header: FileHeader, content: str) -> None:
        ...
        if title:
            view_doc.setMetaInformation(
                QTextDocument.MetaInformation.DocumentTitle, title
            )
    ...
    def is_resource_attached(self, resource_url: str) -> bool:
        ...
        resource = view_doc.resource(
            QTextDocument.ResourceType.ImageResource, QUrl(resource_url)
        )
        if resource:
    ...
    def attach_resource(self, resource_name: str, pixmap: QPixmap) -> bool:
        ...
        view_doc = self.get_view_doc()  # type: QTextDocument
        # url and pixmap data
        view_doc.addResource(
            QTextDocument.ResourceType.ImageResource, resource_name, pixmap
        )
    ...
    def get_cached_resource_pixmap(self, image_url) -> QPixmap:
        ...
        res_folder = self.resource_downloader.get_resource_folder(
            self.get_tree_active_dir()
        )
        ...
        if QPixmapCache.find(image_url, pixmap):
            ...
            if self.resource_downloader.is_external_url(image_url):
                self.logger.debug(
                    f"Resource's '{image_url}' file doesn't exist '{file_path}' yet"
                )
        return pixmap
    ...
    def resource_downloader_finished_handler(self, downloaded_cnt):
        self.logger.debug(
            f"All resource downloader tasks ({downloaded_cnt}) have been completed"
        )
    ...
    def process_document_images(self, text_document):
        ...
        if self.resource_downloader is None:
            ...
            self.resource_downloader.downloaded.connect(
                self.resource_downloaded_handler
            )  # Single resource downloaded
            self.resource_downloader.finished.connect(
                self.resource_downloader_finished_handler
            )  # All tasks finished
        else:
                    ...
                    if image_format.isValid():
                        ...
                        if not self.is_resource_attached(image_url):
                            ...
                            if pixmap.isNull():
                                ...
                                if self.resource_downloader.is_external_url(image_url):
                                    self.resource_downloader.download_resource_in_queue(
                                        image_url
                                    )
                            else:
                                # Attach resource
                            ...
                            self.logger.debug(
                                f"Resource is already attached: {image_url}"
                            )
                it += 1
            block = block.next()
    ...
    def load_content_edit(self, header: FileHeader, content: str) -> None:
        ...
        title = header.get_param("title")
        if title:
            edit_widget.document().setMetaInformation(
                QTextDocument.MetaInformation.DocumentTitle, title
            )
        # Set either default or extended title
        self.set_app_title(title)
    ...
    def get_first_visible_block(
        self, view_widget: QTextBrowser
    ) -> Union[QTextBlock, None]:
    ...
    def store_doc_cursor_pos(self, mode: Mode) -> None:
        ...
        if mode == Mode.EDIT:
            ...
            self.logger.debug(
                "Storing document cursor position %d x %d "
            ...
            self.settings.viewport_pos = [
                view_widget.horizontalScrollBar().value(),
                view_widget.verticalScrollBar().value(),
            ]

    def restore_doc_cursor_pos(
        self, mode: Mode, source_widget: Union[QPlainTextEdit, QTextBrowser]
    ):
        """
        ...
        if mode == Mode.EDIT:
            ...
            self.logger.debug(
                "Restoring document cursor position %d x %d"
            ...
            for _ in range(self.settings.line_num):
                if not text_cursor.movePosition(
            ...
            if self.settings.viewport_pos:
                ...
                if 0 <= self.settings.cursor_pos <= len(source_widget.toPlainText()):
                    ...
                    self.logger.debug(
                        "Cursor's setPosition(%d) is out of range"
                        % self.settings.cursor_pos
                    )
                source_widget.setTextCursor(text_cursor)
                if (
                    isinstance(self.settings.viewport_pos, list)
                    and len(self.settings.viewport_pos) == 2
                ):
    ...
    def get_active_widget(
        self,
    ) -> Union[EditWidget, ViewWidget, QPlainTextEdit, QTextBrowser]:
        if self.get_mode() == Mode.EDIT:
            # Edit widget
            edit_widget = self.get_edit_widget()  # type: Union[EditWidget, QPlainTextEdit]
            return edit_widget
    ...
    def action_search_on(self, searched_text=None) -> None:
        ...
        search_occurrences = search_source.searched_text_count(
            searched_text, find_flags
        )
    ...
    def action_search_case_sensitive(self, state=None) -> None:
        ...
        # Display the count of searched occurrences
        self.toolbar.search_form.set_counter_text(
            str(search_occurrences if search_occurrences > 0 else ("0" if text else ""))
        )
    ...
    def action_search_prev(self) -> None:
        ...
        if search_source.cursor():
            ...
            self.toolbar.search_form.set_position_text(
                index=str(search_source.searched_text_index(pos))
            )
